...
ghes_old_base=$(git merge-base "$ghes_old_sha" v1.18.22)
git rebase --gpg-sign --onto v1.18.22 "$ghes_old_base"
git range-diff "$ghes_old_base...$ghes_old_sha" "v1.18.22...HEAD"
git log --show-signature v1.18.22..HEAD
git diff --check v1.18.22...HEAD
...
```bash
git tag -s -m "GHES distribution v1.18.22-ghes.1" v1.18.22-ghes.1
git push origin refs/tags/v1.18.22-ghes.1
```
