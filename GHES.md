...
curl -LO https://github.com/balcsida/opencode/releases/download/v1.18.22-ghes.1/checksums.txt
curl -LO https://github.com/balcsida/opencode/releases/download/v1.18.22-ghes.1/opencode-linux-x64.tar.gz
...
```yaml
- uses: balcsida/opencode/github@v1.18.22-ghes.1
  with:
    model: openai/gpt-5
    version: v1.18.22-ghes.1
```

`v1.18.22-ghes.1` tracks upstream `v1.18.22`. `v<upstream>-ghes.<revision>` tracks the corresponding upstream OpenCode release, while `latest` resolves to this fork's latest GHES release.
