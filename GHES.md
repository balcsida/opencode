...
curl -LO https://github.com/balcsida/opencode/releases/download/v1.18.16-ghes.3/checksums.txt
curl -LO https://github.com/balcsida/opencode/releases/download/v1.18.16-ghes.3/opencode-linux-x64.tar.gz
...
```yaml
- uses: balcsida/opencode/github@v1.18.16-ghes.3
  with:
    model: openai/gpt-5
    version: v1.18.16-ghes.3
```

`v1.18.16-ghes.3` tracks upstream `v1.18.16`. `v<upstream>-ghes.<revision>` tracks the corresponding upstream OpenCode release, while `latest` resolves to this fork's latest GHES release.
